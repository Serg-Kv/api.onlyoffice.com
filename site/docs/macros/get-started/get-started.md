---
sidebar_position: 1
description: Learn how to create and run macros in ONLYOFFICE editors.
---

# Getting started

Macros are lightweight JavaScript scripts that run directly in ONLYOFFICE documents — no installation required. Write a macro once and run it anytime to automate repetitive tasks across documents, spreadsheets, and presentations.

If you prefer to start coding right away, go directly to the [quick start guides](quick-start/hello-world-macro.md) or try things out in the [interactive playground](playground.md).

## Resources {#resources}

### Development tools

- **[Interactive playground](playground.md)** — Test code without installation
- **[UI component library](https://onlyoffice.github.io/storybook/static/)** — Pre-built components

### Sample code

- **[Macro examples](/samples/?doctype=docs&text=macros)** — Common automation scripts

### Documentation

- **[API reference](/docs/office-api/get-started/overview)** — Complete API documentation
- **[Macro guide](/docs/macros/macros/writing-macros)** — Macro development
- **[AI integration](/docs/ai/ai/ai-plugin)** — AI plugin setup
- **[FAQ](/docs/macros/more-information/faq)** — Frequently asked questions

### Community

- **[Forum](https://forum.onlyoffice.com/)** — Ask questions, share projects
- **[GitHub](https://github.com/ONLYOFFICE/sdkjs-plugins)** — Source code, issues, contributions
- **[Stack Overflow](https://stackoverflow.com/questions/tagged/onlyoffice)** — Technical Q&A
- **[Changelog](/docs/plugins/more-information/changelog)** — Latest API updates

## Next steps {#next-steps}

**Start building:**

- [Quick start guides](quick-start/hello-world-macro.md)
- [Macro tutorial](/docs/macros/macros/getting-started)
- [AI integration](/docs/ai/ai/ai-plugin)

**Experiment:**

- [Open playground](playground.md)
- [Browse examples](https://github.com/ONLYOFFICE/sdkjs-plugins)

**Learn more:**

- [API reference](/docs/office-api/get-started/overview)
- [Writing macros](/docs/macros/macros/writing-macros)
