...
:::tip[Correct]
```json
{ "variations": [{ "icons": ["resources/icon.png", "resources/icon@2x.png"] }] }
```
File structure:
```
my-plugin/
...
```json
// For modal dialog
{ "isModal": true, "isInsideMode": false, "buttons": [{"text": "OK", "primary": true}, {"text": "Cancel"}] }

// For side panel
{ "isModal": false, "isInsideMode": true, "type": "panel" }
```
:::
...
window.Asc.plugin.attachEvent("onSelectionChanged", function(selection) {
  ...
  if (selection && selection.text) { updateUI(selection.text); }
});
```
:::
