...
The skinnable plugin button used in the plugin interface (used only when `type` is `"window"`, `"panel"`, or `"panelRight"`).
...
Plugin icon. Accepts either a template string (recommended) that dynamically resolves to the correct icon based on theme, scale, and state, or a legacy array of icon paths. More information can be found [here](./icons-and-branding.md).

**Example (template string)**: `"resources/%theme-type%(light|dark)/icon%scale%(default).%extension%(png)"`

**Example (legacy array)**: `["icon.png", "icon@2x.png"]`
...
Scalable icons for plugin extensions. More information can be found [here](./icons-and-branding.md).
