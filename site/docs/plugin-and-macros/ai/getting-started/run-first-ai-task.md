---
sidebar_position: 2
---

# Run your first AI task

Once the AI plugin is [installed and configured](installing-ai-plugin.md), you can start using it in any supported editor.

## Using the AI tab

1. Open a document, spreadsheet, or presentation in ONLYOFFICE.
2. Click the **AI** tab in the top toolbar.
3. Choose one of the available actions:
   - **Settings** — configure your AI provider, API key, and model.
   - **Chatbot** — start a conversation to ask questions, rewrite text, or brainstorm ideas.
   - **Summarization** — automatically summarize the entered text and insert the result.
   - **Translation** — translate selected text using the configured AI service.
4. The plugin responds based on the configured AI model.
5. Insert the response into the document or use it as needed.

## Using the context menu

You can also use the AI plugin on selected text:

1. Select text in the editor.
2. Right-click and open the **AI** submenu.
3. Choose one of the options: **Grammar & Spelling**, **Text analysis**, or **Chatbot**.

![AI context menu](/assets/images/plugins/ai-context-menu.png#gh-light-mode-only)![AI context menu](/assets/images/plugins/ai-context-menu.dark.png#gh-dark-mode-only)

## Using the AI agent

For a faster, inline experience, use the AI agent:

1. Press `Ctrl + /` in any editor.
2. Type your request in natural language (e.g., "Summarize this paragraph" or "Make this heading bold").
3. Press `Enter`.

![AI agent prompt](/assets/images/plugins/ai-agent-prompt.png#gh-light-mode-only)![AI agent prompt](/assets/images/plugins/ai-agent-prompt.dark.png#gh-dark-mode-only)

The agent responds immediately and supports multi-step conversation — continue refining with follow-up instructions.

To reset the conversation history, press `Ctrl + Alt + /`. To close the agent, press `Esc`.

For more details on the agent, see [AI agent](../ai-agent/what-is-ai-agent.md).
